...
ENCODER_LIVER_PATH='./trained_models/encoder_18_level4_9798.pth'

WEIGHTS_SAVE_DIR:str='./weights_tumor_encoder'
...
# neural networks & 迁移学习
model = encoding_unetpp.NestedUNet(1,1,)
model.load_state_dict(\
    torch.load(ENCODER_LIVER_PATH,map_location='cpu'))
...
def train_iteration(model:encoding_unetpp.NestedUNet, \
    ...
    for prediction in outputs:
        bce+=weightedBCE(prediction,labels,0.9)
...
if __name__=='__main__':
    ...
    for epoch in range(20):
        ...
        torch.save(model.state_dict(),os.path.join(WEIGHTS_SAVE_DIR,'encoder_{}_level{}_{:04d}.pth'.format(epoch,4,int(dice_score4*10000))))
