...
class NestedUNet(nn.Module):
    ...
    def decode(self, x0_0:torch.Tensor,x1_0:torch.Tensor, \
        ...
        x0_2:torch.Tensor = self.conv0_2(torch.cat([x0_0, x0_1, self.up0_2(x1_1)], dim=1))

        x2_1 = self.conv2_1(torch.cat([x2_0, self.up2_1(x3_0)], dim=1))
        x1_2 = self.conv1_2(torch.cat([x1_0, x1_1, self.up1_2(x2_1)], dim=1))
        x0_3:torch.Tensor = self.conv0_3(torch.cat([x0_0, x0_1, x0_2, self.up0_3(x1_2)], dim=1))
        ...
        x0_4:torch.Tensor = self.conv0_4(torch.cat([x0_0, x0_1, x0_2, x0_3, self.up0_4(x1_3)], dim=1))
